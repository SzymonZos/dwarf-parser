...
from .utils import NoAttributeInDie
...
class Die:
    ...
    def get_die_from_attribute(self, attribute: str) -> DIE:
        try:
            return self._die.get_DIE_from_attribute(attribute)
        except KeyError:
            raise NoAttributeInDie(self)
...
class Type:
    ...
    def get(self):
        try:
            self._die = Die(self._die).get_die_from_attribute('DW_AT_type')
        ...
        try:
            return self.is_special_type() + \
                   Die(self._die).get_attribute_value(
...
class Name:
    ...
    def get(self):
        try:
            return Die(self._die).get_attribute_value('DW_AT_name')
        except NoAttributeInDie:
            try:
                self._die = Die(self._die).get_die_from_attribute(
...
class DieManager:
    def __init__(self, die, prototypes):
        self._die = die
    ...
    def recursively_traverse_dies(self):
        ...
        if die.tag == "DW_TAG_inlined_subroutine":
            die = Die(die.get_die_from_attribute('DW_AT_abstract_origin'))
    ...
    def _process(self):
        return f"{Die(self._die).is_inline()}" \
